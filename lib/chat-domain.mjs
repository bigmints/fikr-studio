import { creationBodyMarkdown, creationDocumentFromArtifact, creationTitleFromMarkdown, ensureCreationDocument } from "./creation-document.mjs";
...
export function canCreateSocialArtifact(queryValue) {
  ...
  const socialTarget = /\b(linkedin|twitter|tweet|social post|social media post|instagram|facebook|caption|x post|post for x|thread for x|x thread|substack|newsletter|medium article|medium post|publish on medium)\b/;
...
export function isKnowledgeInventoryRequest(queryValue, historyValue = []) {
  const query = normalizedText(queryValue);
  const target = /\b(notes|knowledge|workspaces|workspace|spaces|space)\b/;
  const question = /\b(how many|count|total|inventory|breakdown|what do i have)\b/;
  const correction = /\b(wrong|incorrect|not right|actually|i have|there are)\b/;
  if (target.test(query) && (question.test(query) || correction.test(query))) return true;
  if (!/^(wrong|incorrect|that is wrong|that s wrong|not right|no)$/.test(query)) return false;
  const priorUserMessage = [...(Array.isArray(historyValue) ? historyValue : [])]
    .reverse()
    .find((message) => message?.role === "user" && normalizedText(message.content) !== query);
  return priorUserMessage ? isKnowledgeInventoryRequest(priorUserMessage.content) : false;
}

export function buildKnowledgeInventoryAnswer(queryValue, inventoryValue) {
  const inventory = inventoryValue && typeof inventoryValue === "object" ? inventoryValue : {};
  const totalNotes = boundedInteger(inventory.totalNotes, 0, 0, 1_000_000);
  const totalSpaces = boundedInteger(inventory.totalSpaces, 0, 0, 500);
  const noteLabel = totalNotes === 1 ? "note" : "notes";
  const spaceLabel = totalSpaces === 1 ? "Space" : "spaces";
  const lead = inventory.scopeKind === "projects"
    ? `The selected knowledge scope has ${totalNotes} ${noteLabel} across ${totalSpaces} ${spaceLabel}.`
    : `You have ${totalNotes} ${noteLabel} across ${totalSpaces} ${spaceLabel}.`;
  if (!/\b(breakdown|by space|by workspace|each space|each workspace)\b/.test(normalizedText(queryValue))) return lead;
  const rows = (Array.isArray(inventory.spaces) ? inventory.spaces : [])
    .map((space) => `- ${String(space?.name ?? "Untitled workspace").trim().slice(0, 240)}: ${boundedInteger(space?.noteCount, 0, 0, 1_000_000)}`)
    .join("\n");
  return rows ? `${lead}\n\n${rows}` : lead;
}
...
function normalizeWebSources(value) {
  ...
  return value.slice(0, 3).flatMap((candidate, index) => {
    ...
    const urls = normalizeSourceUrls([candidate.requestedUrl, candidate.finalUrl]);
    if (urls.length === 0) return [];
    return [{
      citation: /^W\d+$/.test(String(candidate.citation)) ? String(candidate.citation) : `W${index + 1}`,
      requestedUrl: urls[0],
      finalUrl: urls[1] ?? urls[0],
      title: String(candidate.title ?? "Webpage").trim().slice(0, 500) || "Webpage",
      ...(String(candidate.author ?? "").trim() ? { author: String(candidate.author).trim().slice(0, 300) } : {}),
      ...(String(candidate.siteName ?? "").trim() ? { siteName: String(candidate.siteName).trim().slice(0, 300) } : {}),
      ...(String(candidate.publishedTime ?? "").trim() ? { publishedTime: String(candidate.publishedTime).trim().slice(0, 100) } : {}),
      ...(String(candidate.excerpt ?? "").trim() ? { excerpt: String(candidate.excerpt).trim().slice(0, 500) } : {}),
      wordCount: boundedInteger(candidate.wordCount, 0, 0, 10_000_000),
      fetchedAt: finiteTimestamp(candidate.fetchedAt, Date.now()),
    }];
  });
}

function normalizeDocumentSources(value) {
  if (!Array.isArray(value)) return [];
  return value.slice(0, 120).flatMap((candidate) => {
    if (!candidate || typeof candidate !== "object") return [];
    const attachmentId = cleanId(candidate.attachmentId);
    const name = (String(candidate.name ?? "").trim().split(/[\\/]/).pop() ?? "").slice(0, 180);
    const pageNumber = boundedInteger(candidate.pageNumber, 0, 1, 120);
    const citation = String(candidate.citation ?? "").trim();
    const extractionMethod = candidate.extractionMethod === "ocr" ? "ocr" : "text";
...
/**
 * Builds the bounded, scope-complete knowledge snapshot used by the trusted
 * Electron agent runtime. Ranked retrieval is retained only as a search prior;
 * it must never define the size of the workspace the agent is allowed to see.
 */
export function buildAgentKnowledgeContext(projectsValue, scopeValue, rankedSourcesValue = []) {
  const projects = Array.isArray(projectsValue) ? projectsValue : [];
  const scopedIds = scopeValue?.kind === "projects" && Array.isArray(scopeValue.projectIds)
    ? new Set(scopeValue.projectIds.map(cleanId).filter(Boolean))
    : null;
  const selectedProjects = projects.filter((project) => {
    const projectId = cleanId(project?.id);
    return projectId && (!scopedIds || scopedIds.has(projectId));
  });
  const priorByNoteId = new Map((Array.isArray(rankedSourcesValue) ? rankedSourcesValue : [])
    .map((source, index) => [cleanId(source?.noteId), {
  ...
  const sources = [];
  const spaces = selectedProjects.map((project) => {
    const projectId = cleanId(project.id);
    const projectName = String(project?.name ?? "Untitled workspace").trim().slice(0, 240) || "Untitled workspace";
    ...
    for (const note of blocks) {
      ...
      const prior = priorByNoteId.get(noteId);
      sources.push({
        noteId,
        projectId,
        projectName,
        ...
        score: prior?.score ?? 0,
        priorRank: prior?.rank ?? Number.POSITIVE_INFINITY,
      });
    }
    return { projectId, name: projectName, noteCount: blocks.length };
  });

  sources.sort((left, right) => left.priorRank - right.priorRank
  ...
  return {
    inventory: {
      scopeKind: scopedIds ? "projects" : "all",
      totalNotes: spaces.reduce((total, space) => total + space.noteCount, 0),
      totalSpaces: spaces.length,
      spaces,
    },
    sources: sources.map(({ priorRank: _priorRank, ...source }, index) => ({
      ...source,
...
/** Rebinds runtime citation order to the exact note IDs returned by the agent. */
export function resolveAgentSources(sourcesValue, sourceNoteIdsValue) {
  const sourceById = new Map((Array.isArray(sourcesValue) ? sourcesValue : [])
    .map((source) => [cleanId(source?.noteId), source])
    .filter(([noteId]) => noteId));
  return Array.from(new Set(Array.isArray(sourceNoteIdsValue) ? sourceNoteIdsValue.map(cleanId).filter(Boolean) : []))
    .map((noteId, index) => {
      const source = sourceById.get(noteId);
      return source ? { ...source, citationIndex: index + 1 } : null;
    })
    .filter(Boolean);
}
...
export function buildCitedAnswerFixture(queryValue, sourcesValue) {
  ...
  if (wantsArtifact && sources.length > 0) {
    ...
    const platform = /\bsubstack\b/i.test(query)
      ? "substack"
      : /\bmedium\b/i.test(query)
        ? "medium"
        : /\b(x|twitter|thread)\b/i.test(query)
          ? "x"
          : "linkedin";
    artifact = {
      kind: "social-content",
      platform,
      format: platform === "substack" ? "newsletter" : platform === "medium" ? "article" : /\bthread\b/i.test(query) ? "thread" : "post",
      ...
      hashtags: ["ProductLaunch", "CustomerKnowledge", "BuildInPublic"],
...
function normalizeArtifact(value) {
  ...
  const platform = ["linkedin", "x", "substack", "medium"].includes(value.platform) ? value.platform : "linkedin";
  ...
  const defaultFormats = { linkedin: "post", x: "post", substack: "newsletter", medium: "article" };
  const format = expectedFormats[platform].includes(value.format) ? value.format : defaultFormats[platform];
  ...
  return {
    ...
    ...(String(value.subtitle ?? "").trim() ? { subtitle: String(value.subtitle).trim().slice(0, 240) } : {}),
    content,
    hashtags: Array.isArray(value.hashtags)
      ? Array.from(new Set(value.hashtags.map((tag) => String(tag).trim().replace(/^#+/, "")).filter(Boolean))).slice(0, 5)
      : [],
    ...
    ...(value.skill && typeof value.skill === "object" && cleanId(value.skill.id) && cleanId(value.skill.version)
      ? { skill: { id: cleanId(value.skill.id), version: cleanId(value.skill.version) } }
      : {}),
  };
}

function normalizeInsightDraft(value, hasDocumentSources = false) {
  if (!value || typeof value !== "object") return undefined;
...
function normalizeMessage(value) {
  ...
  const documentSources = role === "assistant" ? normalizeDocumentSources(value.documentSources) : [];
  const artifact = normalizeArtifact(value.artifact);
  const insightDraft = normalizeInsightDraft(value.insightDraft, documentSources.length > 0);
  ...
  return {
    ...
    sourceNoteIds: Array.isArray(value.sourceNoteIds)
      ? value.sourceNoteIds.map(cleanId).filter(Boolean).slice(0, 20)
      : [],
    webSources: role === "assistant" ? normalizeWebSources(value.webSources) : undefined,
    documentSources: role === "assistant" ? documentSources : undefined,
    outputKind,
...
export function createCreationFromArtifact({
  ...
  if (!artifact) throw new Error("A valid social-content artifact is required");
  const safeNow = finiteTimestamp(now, Date.now());
  const safeThreadId = slug(threadId);
  const id = uniqueId(`creation-${safeThreadId}-${safeNow}`, existingIds);
  const outputMarkdown = creationDocumentFromArtifact(artifact);
  return {
    id,
    name: creationTitleFromMarkdown(outputMarkdown, artifact.title),
    mode: "article",
...
export function creationMatchesArtifact(creationValue, artifactValue) {
  ...
  const content = creationBodyMarkdown(creationValue.outputMarkdown ?? "");
  const artifactContent = creationBodyMarkdown(ensureCreationDocument(artifact.content, artifact.title));
  const platform = ["linkedin", "x", "substack", "medium"].includes(creationValue.platform) ? creationValue.platform : "linkedin";
  const format = String(creationValue.format ?? (platform === "substack" ? "newsletter" : platform === "medium" ? "article" : "post"));
  return content === artifactContent && platform === artifact.platform && format === artifact.format;
}
