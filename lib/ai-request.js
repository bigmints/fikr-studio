...
const PROVIDER_VERIFY_URLS = Object.freeze({
...
async function verifyAiKey({ provider, apiKey, fetchImpl = fetch, timeoutMs = 15_000 }) {
  const url = PROVIDER_VERIFY_URLS[provider];
  ...
  const timer = setTimeout(() => controller.abort(), timeoutMs);
  try {
    const headers = { Accept: 'application/json', Authorization: `Bearer ${apiKey.trim()}` };
    if (provider === 'openrouter') {
      headers['HTTP-Referer'] = 'https://fikr.one';
      headers['X-Title'] = 'Fikr Studio';
    }
    ...
    if (Buffer.byteLength(responseBody, 'utf8') > MAX_RESPONSE_BYTES) {
      throw new Error('AI verification response exceeds 5 MB');
    }
