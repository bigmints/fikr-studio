 ...
 * @param {object} [prevWorkspace]  Previous state for diffing (optional)
 */
async function saveWorkspace(userId, workspace, prevWorkspace) {
  ...
  for (const proj of incomingProjects) {
    if (!dbProjectIds.has(proj.id)) {
      ...
      await _syncNotes(userId, newId, [], proj.blocks ?? [], proj.ghostNotes ?? []);
    }
  }
  ...
  for (const proj of incomingProjects) {
    ...
    await _syncNotes(userId, proj.id, [...dbNotes, ...dbGhosts], proj.blocks ?? [], proj.ghostNotes ?? []);
  }
 ...
 */
async function _syncNotes(userId, projectId, dbNotes, blocks, ghostNotes) {
  const allIncoming = [
  ...
  // Delete notes removed from local state
