...
async function listProjects(userId) {
  const parseProjects = (snap) =>
    snap.docs.map(d => ({ id: d.id, ...d.data() })).filter(p => !p.archivedAt);
  ...
  try {
    const snap = await db()
      ...
      .get();
    return parseProjects(snap);
  } catch (e) {
    // FAILED_PRECONDITION = composite index missing. Fall back to unordered fetch
    // so sync still works rather than blowing up and loading stale disk data.
    if (e.code === 9 /* FAILED_PRECONDITION */ || (e.message && e.message.includes('index'))) {
      console.warn('[Firestore] listProjects: composite index missing — falling back to unordered query. Create index at:', e.message.match(/https:\/\/[^\s]+/)?.[0] ?? 'Firebase console');
      const snap = await db()
        .collection(PROJECTS)
        .where('userId', '==', userId)
        .get();
 ...
 * @param {Set<string>} [lastSyncedNoteIds]    Baseline of note IDs seen on last cloud load.
 * @param {Set<string>} [lastSyncedProjectIds] Baseline of project IDs seen on last cloud load.
 * @param {Set<string>} [lastSyncedGenProjectIds] Baseline of gen project IDs seen on last cloud load.
 *   All are used to distinguish "deleted locally" from "created on another device".
 *   If not provided, deletions are skipped entirely (safe fallback).
 */
async function saveWorkspace(userId, workspace, lastSyncedNoteIds, lastSyncedProjectIds, lastSyncedGenProjectIds) {
  ...
  // 3. Archive removed projects — ONLY if we previously knew about them AND the
  //    user explicitly removed them locally (i.e. they were in our project baseline).
  //    A project missing from local state but NOT in lastSyncedProjectIds was created
  //    on another device — we must NOT archive it.
  for (const dbProj of dbProjects) {
    if (!incomingProjectIds.has(dbProj.id)) {
      if (lastSyncedProjectIds && lastSyncedProjectIds.has(dbProj.id)) {
        await deleteNotesByProject(dbProj.id);
        await archiveProject(dbProj.id);
      }
      // else: project exists in cloud but wasn't in our baseline → another device created it → skip
    }
  }

  // 4. Sync Studio generation projects (separate collection)
  if (workspace.studioProjects !== undefined) {
    await _syncGenProjects(userId, workspace.studioProjects, lastSyncedGenProjectIds);
  }
}
...
async function listGenProjects(userId) {
  const parseGenProjects = (snap) => snap.docs.map(d => ({ id: d.id, ...d.data() }));
  ...
  try {
    const snap = await db()
      ...
      .orderBy('createdAt', 'asc')
      .get();
    return parseGenProjects(snap);
  } catch (e) {
    if (e.code === 9 /* FAILED_PRECONDITION */ || (e.message && e.message.includes('index'))) {
      console.warn('[Firestore] listGenProjects: composite index missing — falling back to unordered query. Create index at:', e.message.match(/https:\/\/[^\s]+/)?.[0] ?? 'Firebase console');
      const snap = await db()
        .collection(GEN_PROJECTS)
        .where('userId', '==', userId)
        .get();
 ...
 */
async function _syncGenProjects(userId, incomingProjects, lastSyncedGenProjectIds) {
  ...
  for (const doc of snap.docs) {
    if (!incomingIds.has(doc.id)) {
      if (lastSyncedGenProjectIds && lastSyncedGenProjectIds.has(doc.id)) {
