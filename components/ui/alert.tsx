  ...
  "relative w-full rounded-lg border px-4 py-3 text-sm grid has-[>svg]:grid-cols-[calc(var(--spacing)*4)_1fr] grid-cols-[0_1fr] has-[>svg]:gap-x-3 gap-y-0.5 items-start [&>svg]:translate-y-0.5 [&>svg]:text-current",
  {
    variants: {
      variant: {
        ...
        destructive:
          "text-destructive-foreground border-destructive/30 bg-destructive/10 [&>svg]:text-destructive",
        primary:
          "border-primary/30 bg-primary/8 text-foreground [&>svg]:text-primary",
        warning:
          "border-yellow-500/30 bg-yellow-500/8 text-foreground [&>svg]:text-yellow-500",
      },
    },
    defaultVariants: {
      variant: "default",
