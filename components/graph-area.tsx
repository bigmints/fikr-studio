...
export function GraphArea({
    ...
    simRef.current = d3
      ...
      .force(
        ...
        d3
          ...
          .strength((l) => ((l as SimLink).isSynthesisLink ? 0.03 : 0.3)),
      )
      .force(
        "charge",
        d3
          .forceManyBody<SimNode>()
          .strength((n) => (n.isSynthesis ? -700 : -420)),
      )
      .force(
        "collide",
        d3
          .forceCollide<SimNode>()
          .radius((n) => calcR(n.degree, 1) + 30)
          .strength(0.88),
      )
      .alphaDecay(0.012)
      .velocityDecay(0.38)
  ...
  React.useEffect(() => {
    ...
    const prevBlockCount = nodesRef.current.filter(
      (n) => !n.isSynthesis,
    ).length;
    ...
    sim.force(
      ...
      d3
        ...
        .strength((n) => (n.isSynthesis ? 0.25 : 0.1)),
    );
    ...
    (sim.force("link") as d3.ForceLink<SimNode, SimLink>).links(
      linksRef.current,
    );
        ...
        >
          <defs>
            ...
            <radialGradient id="synth-grad" cx="50%" cy="50%" r="50%">
              <stop
                offset="0%"
                stopColor="var(--type-thesis)"
                stopOpacity="1"
              />
              <stop
                offset="100%"
                stopColor="var(--type-claim)"
                stopOpacity="0.8"
              />
            </radialGradient>
          </defs>
          ...
          <g transform={`translate(${tx},${ty}) scale(${tk})`}>
            ...
            <g>
              {linksRef.current.map((link, i) => {
                ...
                const dimmed =
                  focalId != null && s.id !== focalId && t.id !== focalId;
                ...
                return (
                  <path
                    ...
                    strokeOpacity={
                      dimmed ? 0.02 : highlighted ? 0.75 : isSynth ? 0.05 : 0.22
                    }
                    fill="none"
                    style={{
            ...
            <g>
              {nodesRef.current.map((node) => {
                ...
                const r = node.isSynthesis
                  ? R_SYNTH
                  : calcR(node.degree, maxDeg);
                ...
                const labelWords = (node.block?.text ?? "")
                  .split(/\s+/)
                  .slice(0, 4)
                  .join(" ");
                const label =
                  labelWords.length > 22
                    ? labelWords.slice(0, 22) + "…"
                    : labelWords;
                  ...
                  >
                    ...
                    {node.id === highlightedBlockId &&
                      ...
                      !isSelected && (
                        <circle
                          ...
                          fill="none"
                          stroke={
                            node.isSynthesis ? "var(--type-thesis)" : accent
                          }
                          strokeWidth={1.2}
                          strokeOpacity={0.55}
                        />
                    ...
                    {isEnriching && (
                      <circle
                        ...
                        style={{
                          transformBox:
                            "fill-box" as React.CSSProperties["transformBox"],
                    ...
                    <circle
                      r={r}
                      fill={fill}
                      fillOpacity={isSelected ? 1 : isHovered ? 0.97 : 0.9}
