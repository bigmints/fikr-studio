...
export function GraphDetailPanel({
  ...
  const headerBg =
    block.contentType === "thesis"
      ...
      : block.isPinned
        ...
        : accent;
  const headerColor =
    block.contentType === "thesis" ? "var(--thesis-foreground)" : "black";
  ...
  const commitAnnotation = () => {
    ...
    if (trimmed !== (block.annotation ?? ""))
      onEditAnnotation(block.id, trimmed);
    setEditingAnnotation(false);
  };
    ...
    <div className="flex h-full flex-col overflow-hidden border-l border-border/60 bg-card">
      ...
      >
        ...
        >
          ...
          <span
            className={`font-mono text-[10px] font-bold uppercase tracking-wider ${block.isEnriching ? "shimmer-text" : ""}`}
          >
            {config.label}
          </span>
          ...
          <div className="px-4 pb-3 space-y-1">
            ...
            <div className="h-1 w-full rounded-full bg-secondary overflow-hidden">
              <div
                ...
                style={{ width: `${block.confidence}%`, background: accent }}
              />
            </div>
          </div>
        )}
          ...
          <div className="px-4 pb-4 space-y-1.5">
            ...
            <div className="space-y-1">
              {connectedBlocks.map((b) => {
                const bConfig =
                  CONTENT_TYPE_CONFIG[b.contentType] ||
